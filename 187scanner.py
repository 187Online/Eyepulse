"""
If you get scapy pcap error install wireshark

Perform a slow scan to avoid detection by the firewall

To avoid detection by the firewall, the timeout value should be between 1-5 seconds(TCP/ACK,STEALTH/ZOMBIE)

******* 187SC4NN3R *******   
    
Developed by 187Online  1/03/2023
...
WARNING,ON_PROC,STARTING,END =  '\033[1;91m', '\33[1;97m','\033[1;32m', '\033[0m'
...
class PortScanner : 
    ...
    def __init__(self) : 
        ...
        print(STARTING+ '******187SC4NN3R STARTED SUCCESSFULY*******'+ END )
    ...
    def net_show(self):
        ...
        for addr in self.ping_true_list :
            try : 
                ...
                network_table.add_row([addr,WARNING+'NONE'+ END])
        print(network_table)
    ...
    def fast_scan(self,server_addr,sleep_time=0):  
        sys.stdout.flush()
        print(WARNING + 'THIS METHOD USE ONLY COMMON PORTS IF NO RESULTS USE FULL SCAN OR RANGE SCAN' + END)
        print(STARTING +' [I] FAST SCAN STARTED... '+ END)
        ...
        try: 
            for port in common_ports :
                ...
                if self.open_port_list  :
                    print(WARNING+f'Open port list = {self.open_port_list}'+END +ON_PROC +f'scannıng on process {port}\r'+END,end="")
        
                else :
                    print(ON_PROC + f'{port} not open\r' + END,end ="")
    ...
    def full_scan(self,server_addr,sleep_time=0) :
        ...
        print(WARNING + '''
        List contains 'useful' ports
        Look at the port list, if there are no ports you have specified,use add functıon or try to range scan 
        For looking port list use list_ports 
        '''+END)
    ...
    def range_scan(self,server_addr,port_range=65535,sleep_time=0) : 
        sys.stdout.flush()
        print(WARNING + ' Thıs process take very long tıme ' + END )
        print(STARTING + '[I] Range scan has been started...' +END )
        port_range = int(port_range)
        try :
            for port in range(1,port_range+1):
                response = self.scan(server_addr,port)
                if response == 0 : 
                    print(WARNING + f'port open {port}' + END)
                    self.open_port_list.append(port)
                else : 
                    print(ON_PROC + f'{port} not open\r' + END,end="")
                time.sleep(sleep_time)
    ...
    def ping_scan(self) :
        print(WARNING + 'THIS ARP PING METHOD WORKS SLOW IF YOU DONT WAIT USE  "net_recon" ' + END)
                     
        gateway = conf.route.route('0.0.0.0')[2]
        
        print(WARNING+f'ROUTER ADDRESS  : {gateway}'+END)
        print(STARTING + ' [I] PING SCAN STARTED... ' + END) 
        ...
        while int(gateway[3]) <= 255 : 
            ...
            if response.success() :
                print(WARNING +f'{gateway} ON NETWORK ' + END)
    ...
    def ICMP_scan(self) :
        print(STARTING +' [I] ICMP SCAN WITH SCAPY STARTED.....' + END )    
        for ip in range(1,256) :
            try:
                packet = IP(dst='192.168.1.' +str(ip),ttl=20)/ICMP()
                response = sr1(packet,timeout=1,verbose=False)
                if response.dst == packet.src: 
                    print(WARNING + 'ON NETWORK = ' + '192.168.1.'+str(ip) + END)
                    self.ping_true_list.append(response.src)
            
            except AttributeError as NoneTypeError :
                print(ON_PROC + f'No response from 192.168.1.'+str(ip) + END)
        
        self.net_show()
            
   
    def net_recon(self) : 
        print(STARTING+'FASTEST ARP PING METHOD' +END)
    ...
    def stealth_scan(self,server_address,srcport,time_out) :
        #Default tımeout value = 1 
        sys.stdout.flush()
        print(STARTING +'[I] SYN Scan Started ........'+ END) 
        def probe_port(port):
            syn_packet = IP(dst=server_address)/TCP(sport=srcport,dport=port,flags='S') 
            ...
            if response is not None :
                if response.haslayer(TCP) :    
                    if response.getlayer(TCP).flags == 0x12:
                        ...
                        print(ON_PROC +f'port closed {port}'+ END)
    ...
    def tcp_ack_scan(self,server_address,srcport,time_out,seq_number) :
        print(WARNING + 'TCP ACK SCAN HELPS TO DETERMINE UNFILTERED PORTS'+ END)
        print(STARTING + 'ACK SCAN STARTED....'+ END)
        
        def probe_port(port) :   
            try :
                ack_packet = IP(dst=server_address)/TCP(sport=srcport,dport=port,flags='A',seq=seq_number) 
                response = sr1(ack_packet,timeout=time_out,verbose=False)
                if response is None : 
                    print(ON_PROC +f'PORT FILTERED {port}\r'+ END,end="")
                elif response is not None :
                    if response.haslayer(TCP) :
                        if  response.getlayer(TCP).flags == 'R' :
                            print(WARNING +f'PORT UNFILTERED {port}'+END)
                            self.unfiltered_port_list.append(port)
                    if response.haslayer(ICMP) : 
                                if(int(response.getlayer(ICMP).type)==3 and int(response.getlayer(ICMP).code) in [1,2,3,9,10,13]):
                                    print(WARNING + f'FILTERED PORT(ICMP ERROR)  = {port}' + END )
    ...
    def zombie_scan(self,zombie,victim,zombie_port,time_out) :
        print(WARNING + 'MAKE SURE THAT THE MACHINE YOU HAVE TURNED INTO A ZOMBIE HAS THE LEAST AMOUNT OF TRAFFIC ON THE NETWORK ' + END)
        print(STARTING + ' [I] ZOMBIE SCAN STARTED...' + END)
                    ...
                    if evil_response.getlayer(TCP).flags == 'R'  :      
                        ...
                        if zombie_final_response is not None :
                            if  zombie_final_response[IP].id ==  evil_response[IP].id + 2 :      
                                ...
                                print( WARNING + f'port open {victim_port}' + END )
                                
                            else : 
                                print(ON_PROC + f'port closed or filtered {victim_port}\r' + END,end="")
                        else : 
                            print(WARNING +'from zombie couldnt receive the last packet' +END )
            except AttributeError as NoneTypeError : 
    ...
    def udp_scan(self,server_address,time_out):
        print(WARNING +' UDP scanning generates less traffic compared to TCP scanning, and therefore the scanning process is less noticeable in network traffic'+ END)
        print(STARTING + '[I] UDP SCAN STARTED....'+END)
        
        def probe_udp(port):
            udp_packet = IP(dst=server_address)/UDP(dport=port) 
            response =sr1(udp_packet,timeout=time_out,verbose=False)
            if response is None : 
                print(WARNING + f'Port Closed/Filtered {port}'+ END)
                self.udp_closed_filtered_list.append(port)
            else :
                if response.haslayer(ICMP) :
                    print(ON_PROC+f'PORT CLOSED {port}\r'+END,end="")
                elif response.haslayer(UDP):
                    print(WARNING+f'PORT OPEN {port}'+END)
                    self.udp_open_lists.append(port)
                else : 
                    print(ON_PROC+f'FILTERED(ICMP UNREACHABLE) {port}'+END)
...
if __name__ == '__main__': 
    ...
    """ METHODS """
    parser.add_argument('--get_ip',action='store_true',help ='domain to ip address | Requireds --url')
    parser.add_argument('--icmpsc',action='store_true',help ='control the server status (up/down) | Required --server_address ')
    parser.add_argument('--spl',action='store_true',help ='show port list | Required None ')
    parser.add_argument('--fast_scan',action='store_true',help ='uses common ports | Required --server_address | Defaults = --sleep_time 0 ')
    parser.add_argument('--full_scan',action='store_true',help ='scans all "useful" ports  | Required --server_address | Defaults = --sleep_time 0 ')
    parser.add_argument('--range_scan',action='store_true',help ='1 to 65535  | Required --server_address | Defaults = --port_range 65535 , --sleep_time 0')
    parser.add_argument('--ping_scan',action='store_true',help ='arp ping method for probe local network devices | Required None (LOCAL)')
    parser.add_argument('--icmp_scan',action='store_true',help ='icmp method for probe local network devices | Required None (LOCAL) ')
    parser.add_argument('--net_recon',action='store_true',help ='arp ping method (fast way to probe local network devices) | Required None (LOCAL)')
    parser.add_argument('--stealth_scan',action='store_true',help ='syn scan | Required --server_address | Defaults = --srcport 80  , --time_out 3')
    parser.add_argument('--tcp_ack_scan',action='store_true',help ='probe unfiltered ports | Required --server_address | Defaults = --srcport 80 , --timeout 3 --seq_number 666')
    parser.add_argument('--zombie_scan',action='store_true',help ='zombify the least trafficked device on the network and perform port scanning through it. | Required --zombie --victim | Defaults = --zombie_port 80 ,--time_out 3' )
    parser.add_argument('--udp_scan',action='store_true',help ='simple udp scan | Required --server_address | Defaults --time_out 3')
    """ARGUMENTS """ 
    parser.add_argument('--url',action='store',help ='url addres for get_ip function ')
    parser.add_argument('--server_address',action='store',help ='server address for scan types ')
    parser.add_argument('--sleep_time',action='store',type=float,default=0,help ='slow scanning is a more humane approach to detecting firewalls (default=0 ) ')
    parser.add_argument('--port_range',action='store',type=int,default=65535,help ='port range for scan ')
    parser.add_argument('--srcport',action='store',type=int,default=80,help ='source port(default=80)')
    parser.add_argument('--time_out',action='store',type=float,default=3,help ='timeout value for TCP/ACK/STEALTH/ZOMBIE SCAN')
    parser.add_argument('--seq_number',action='store',type=int,default=666,help ='To leave a different impression from other scans, use a different SEQ number')
    parser.add_argument('--zombie',action='store',help ='zombie machine ip address for zombie scan ')
    parser.add_argument('--victim',action='store',help ='victim machine ip address for zombie scan ')
    parser.add_argument('--zombie_port',action='store',type=int,default=80,help ='zombie machine port adress ')
    ...
    if args.stealth_scan:
        scanner.stealth_scan(args.server_address,args.srcport,args.time_out)
    
    if args.tcp_ack_scan:
        scanner.tcp_ack_scan(args.server_address,args.srcport,args.time_out,args.seq_number)
